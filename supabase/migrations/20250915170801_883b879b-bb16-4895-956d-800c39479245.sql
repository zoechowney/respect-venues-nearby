-- Enhanced Security for Application Tables with Personal Data Protection
...
-- Create audit log table for tracking access to sensitive application data
...
-- Drop existing permissive INSERT policies that are too broad
...
-- Create new, more secure INSERT policies with rate limiting
...
-- Strengthen existing SELECT policies with explicit field protection
-- Replace the existing venue application SELECT policies with more explicit ones
...
-- Venue owners can only see limited fields of their own applications
...
AS $$
BEGIN
  -- Log access to applications containing personal data
  INSERT INTO application_audit_log (
    ...
    NEW.id,
    TG_OP,
    ARRAY['email', 'phone', 'contact_name'],
    inet_client_addr()
  );
...
-- Add audit triggers for sensitive data access
CREATE TRIGGER audit_venue_applications_access
  AFTER SELECT ON venue_applications
  FOR EACH ROW
  EXECUTE FUNCTION public.log_sensitive_data_access();

CREATE TRIGGER audit_sponsor_applications_access
  AFTER SELECT ON sponsor_applications
  FOR EACH ROW
  EXECUTE FUNCTION public.log_sensitive_data_access();

-- Add constraints to prevent malicious data injection
ALTER TABLE venue_applications 
ADD CONSTRAINT check_email_length CHECK (char_length(email) <= 254),
ADD CONSTRAINT check_phone_format CHECK (phone IS NULL OR phone ~* '^[\+]?[1-9][\d\s\-\(\)]{7,15}$'),
ADD CONSTRAINT check_business_name_length CHECK (char_length(business_name) <= 200),
ADD CONSTRAINT check_contact_name_length CHECK (char_length(contact_name) <= 100);

ALTER TABLE sponsor_applications 
ADD CONSTRAINT check_sponsor_email_length CHECK (char_length(email) <= 254),
ADD CONSTRAINT check_sponsor_phone_format CHECK (phone IS NULL OR phone ~* '^[\+]?[1-9][\d\s\-\(\)]{7,15}$'),
ADD CONSTRAINT check_company_name_length CHECK (char_length(company_name) <= 200),
ADD CONSTRAINT check_sponsor_contact_name_length CHECK (char_length(contact_name) <= 100);
...
-- Update existing update policies to be more explicit about admin-only access
