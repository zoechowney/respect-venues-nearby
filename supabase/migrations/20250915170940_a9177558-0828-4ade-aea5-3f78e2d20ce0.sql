-- Enhanced Security for Application Tables with Personal Data Protection (Final Fix)
...
-- Strengthen SELECT policies - ensure only authorized access to personal data
...
-- Create function to log sensitive data changes
...
-- Add validation constraints (check if they don't already exist)
DO $$
BEGIN
  -- Add email length constraint for venue_applications
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'check_email_length' 
    AND table_name = 'venue_applications'
  ) THEN
    ALTER TABLE venue_applications 
    ADD CONSTRAINT check_email_length CHECK (char_length(email) <= 254);
  END IF;

  -- Add phone format constraint for venue_applications
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    ...
    ALTER TABLE venue_applications 
    ADD CONSTRAINT check_phone_format CHECK (phone IS NULL OR phone ~ '^[\+]?[0-9\s\-\(\)\.]{7,20}$');
  END IF;

  -- Add business name length constraint for venue_applications
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
  ...
  -- Add contact name length constraint for venue_applications
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'check_contact_name_length' 
    AND table_name = 'venue_applications'
  ) THEN
    ALTER TABLE venue_applications 
  ...
  -- Add constraints for sponsor_applications
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'check_sponsor_email_length' 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'check_sponsor_phone_format' 
    AND table_name = 'sponsor_applications'
  ) THEN
    ALTER TABLE sponsor_applications 
    ADD CONSTRAINT check_sponsor_phone_format CHECK (phone IS NULL OR phone ~ '^[\+]?[0-9\s\-\(\)\.]{7,20}$');
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'check_company_name_length' 
    AND table_name = 'sponsor_applications'
  ) THEN
    ALTER TABLE sponsor_applications 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'check_sponsor_contact_name_length' 
    AND table_name = 'sponsor_applications'
  ) THEN
    ALTER TABLE sponsor_applications 
